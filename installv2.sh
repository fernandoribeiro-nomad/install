...
echo "Usuário: $ACTIVE_USER | Versão: $UBUNTU_CODENAME"
echo "--------------------------------------------------"

# 1. FERRAMENTAS ESSENCIAIS (Somente o básico do sistema)
...
if [ $? -eq 0 ]; then
    ...
    ERROS_DETALHADOS+=("Passo 1: Falha ao instalar ferramentas básicas via APT.")
fi
...
if [ -f "/tmp/anydesk.deb" ]; then
    ...
    echo -e "${VERMELHO}fail${NC}"
    ERROS_DETALHADOS+=("Passo 5: Falha ao baixar o .deb do AnyDesk.")
fi
...
# 10. NETSKOPE (Com dependências próprias incluídas)
...
if pgrep -if $appTarget > /dev/null; then
    ...
    # Inicia processo de instalação conforme solicitado
    apt-get update > /dev/null 2>&1
    # Instala as dependências específicas aqui
    ...
    # Lógica de coleta de usuário e ativação (Exatamente como o script original)
    USER_NET=$(users | awk '{print $1}')
    IDUSER_NET=`id $USER_NET | awk -F'[=($]' '{print $2}'`
    
    # Ativação do agente
    su -c "XDG_RUNTIME_DIR="/run/user/$IDUSER_NET" DBUS_SESSION_BUS_ADDRESS="unix:path=${XDG_RUNTIME_DIR}/bus" systemctl --user --now enable stagentapp.service" $USER_NET > /tmp/netskope_error.log 2>&1
    ...
    if pgrep -if "stAgentApp" > /dev/null; then
        ...
        ERROS_DETALHADOS+=("Passo 10: Netskope instalado, mas stAgentApp não iniciou. Erro: ${MOTIVO:-"Unit file not found ou falha de Bus"}")
    fi
fi
...
if [ -f "/tmp/$WAZUH_PKG_NAME" ]; then
    ...
    echo -e "${VERMELHO}fail${NC}"
    ERROS_DETALHADOS+=("Passo 12: Falha no download do pacote Wazuh.")
fi
...
if [ -f "$GDM_CONFIG" ]; then
    ...
    echo -e "${VERMELHO}fail${NC}"
    ERROS_DETALHADOS+=("Passo 13: Arquivo custom.conf não encontrado.")
fi
